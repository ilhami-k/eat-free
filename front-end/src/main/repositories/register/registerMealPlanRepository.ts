...
export function registerMealPlanRepository() {
  ...
  ipcMain.handle("mealPlanRepository:getMealPlans", (e, userId: number) => {
    return mealPlanRepository.getMealPlans(userId);
  });

  ipcMain.handle("mealPlanRepository:getMealPlanById", (e, id: number) => {
    return mealPlanRepository.getMealPlanById(id);
  });

  ipcMain.handle(
    "mealPlanRepository:getMealPlanForWeek",
    (e, userId: number, weekStartDate: Date) => {
  ...
  ipcMain.handle(
    ...
    (e, user_id: number, week_start_date: Date) => {
      return mealPlanRepository.createMealPlan(user_id, week_start_date);
    }
  );

  ipcMain.handle("mealPlanRepository:deleteMealPlan", (e, id: number) => {
    return mealPlanRepository.deleteMealPlan(id);
  });

  ipcMain.handle(
    "mealPlanRepository:addRecipeToMealPlan",
    (e, plan_id: number, recipe_id: number, date: Date, meal_type: meal_plan_recipe_meal_type, planned_servings: number) => {
  ...
  ipcMain.handle(
    ...
    (e, id: number, planned_servings: number) => {
