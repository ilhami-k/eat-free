...
BEGIN
  ...
  IF p_deduct_inventory THEN
    SELECT id INTO v_inv_id FROM Inventory WHERE user_id = p_user_id;
    ...
    WHERE ii.inventory_id = v_inv_id;
    ...
    LEFT JOIN Inventory_Ingredient ii
      ON ii.inventory_id = v_inv_id AND ii.ingredient_id = need.ingredient_id
...
-- 2) Normalize Meal_Plan.week_start_date to MONDAY on INSERT/UPDATE
...
-- 3) Keep Recipe nutrition cache up-to-date whenever recipe lines change
